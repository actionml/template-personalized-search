...
**Behavioral Search**: The general case of using CCO to augment search indexes uses all indicators we can create, including the primary conversion indicator and any secondary ones that can be derived. It is "personalize" because you use not only the terms in the search box but user history, context, or profile information.

**Dataflow**: The template is essentially the same as the UR but outputs csvs of text fields that can be added to whatever is used in the applications search, which often means Solr, Elasticsearch, etc. This means the input is event encoding of user conversions, and other indicators of user preferences--including search terms, then a training task creates the output csvs. The template does not implement the search or directly write to some index. It is expected that the user of the template will do this part of the dataflow.
