...
Personalized Search is based on the same algorithm used in the Universal Recommender called the Correlated Cross-Occurrence algorithm. CCO is able to find correlations between the searches people make and the things they buy, watch, listen-to, or read (called a conversion). It can also find behavioral data that correlates with these desired action. In the UR these are used to make recommendations, in Augmented or Behavioral Search they are output to be added to the item search index. 

**Augmented Search**: A special case of CCO that finds search terms that people have used that correlate with purchases or any action that is deemed a "conversion". These terms may not be contained in the content of the items but have, in a significant way, led to the conversion event in the past. When these terms are added to a search index and the query is made against the content but also the augmented field, searches will generate lift to conversions from searches. This is not personalized because it doesn't use user history.

**Personalized Search**: The general case of using CCO to augment search indexes uses all indicators we can create, including the primary conversion indicator and any secondary ones that can be derived. It is "personalized" because you use not only the terms in the search box but user history, context, or profile information. So this technique includes all the benefits of Augmented Search but uses user data to personalize. 

**Dataflow**: The template is essentially the same as the UR but outputs fields in a form that can be added to whatever is used as the application search engine, which often means Solr, Elasticsearch, etc. This means the input is event encoding of user conversions, and other indicators of user preferences&mdash;including search terms, then a training task creates the output files. The template does not implement the search or directly write to some index. It is expected that the user of the template will have their own search engine and will modify how queries are constructed to use the new fields created with this template.

The rest of this doc is based on the UR and will be updated over time.
