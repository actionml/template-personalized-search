...
class URModel(
  ...
  def save( params: URAlgorithmParams): Boolean = {
    ...
    // If we are savng the indicators as text them let's not bother writing to Elasticsearch.
    // This is for using the UR as a Personalized search template.
    if (params.modelFilename.nonEmpty && coocurrenceMatrices.nonEmpty) {
      logger.info("Writing model to text files for Personalized search, no UR Recommender model created.")
      writeToFile(coocurrenceMatrices.get, params.modelFilename.get)
    } else {
  ...
  /** Write the correlators to a csv **/
  def writeToFile(modelMatrices:  List[(String, IndexedDataset)], filename: String): Unit = {
    logger.info("Writing model to " + filename )
    val fieldDelim = "\t"
    val elementDelim = ":"
    modelMatrices.foreach { case (actionName, dataset) =>
      dataset.asInstanceOf[IndexedDatasetSpark].toStringMapRDD(actionName).map { case (item, indicators) =>
        item + fieldDelim + indicators(actionName).mkString(elementDelim)
      }.saveAsTextFile(filename + "." + actionName)
    }
  }
